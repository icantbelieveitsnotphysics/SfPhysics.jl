...
"""
function diffraction_limited_spot_diameter(r::Unitful.Length, d_element::Unitful.Length, λ::Unitful.Length)
	if r <= 0u"m"
	...
	return 2λ / (π * atan(d_element, 2r)) |> unit(d_element)
end

function diffraction_limited_element_size(r::Unitful.Length, d_spot::Unitful.Length, λ::Unitful.Length)
	if r <= 0u"m"
		throw(DomainError(r, "Range r must be greater than zero"))
	elseif d_spot <= 0u"m"
...
function diffraction_limited_range(d_element::Unitful.Length, d_spot::Unitful.Length, λ::Unitful.Length)
	if d_element <= 0u"m"
		throw(DomainError(d_element, "Element diameter must be greater than zero"))
	elseif d_spot <= 0u"m"
		throw(DomainError(d_spot, "Spot diameter must be greater than zero"))
...
function diffraction_limited_wavelength(r::Unitful.Length, d_element::Unitful.Length, d_spot::Unitful.Length)
	if d_element <= 0u"m"
		throw(DomainError(d_element, "Element diameter must be greater than zero"))
	elseif d_spot <= 0u"m"
		throw(DomainError(d_spot, "Spot diameter must be greater than zero"))
	...
	return tan(d_element /2r) * π * d_spot / 2 |> u"nm"
end
