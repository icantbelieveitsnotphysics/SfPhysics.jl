...
const venus = Body(
			"Venus", 4.8675u"kg" * big(10)^24, Sphere(6051.8u"km"), 0.76,
			Orbit(sol, 0.723332u"AU", 0.006772, 3.86, 76.680, 54.884),
			Rotation(nothing, -324.025u"d", 177.36))

const earth = Body(
			"Earth", 1u"Mearth", Spheroid(1u"Rearth_e", 1u"Rearth_p"), 0.306,
...
const moon = Body(
			"Moon", 7.34767309u"kg" * big(10)^22, Sphere(1737.1u"km"), 0.136, # not clear if this is bond albedo
			Orbit(earth, 384399u"km", 0.0549,  5.145u"°"),
			Rotation(0.3929, 27.321661u"d", 24))

const mars = Body(
			"Mars", 6.4171u"kg" * big(10)^23, Spheroid(3396.2u"km", 3376.2u"km"), 0.25,
			Orbit(sol, 227939200u"km", 0.0934, 5.65, 49.558, 286.502),
			Rotation(0.3644, 1.025957u"d", 25.19))
const phobos = Body(
			"Phobos", 1.0659u"kg" * big(10)^16, Ellipsoid(13.5u"km", 11u"km", 9u"km"), 0.071, # not clear if this is bond albedo
			Orbit(mars, 9376u"km", 0.0151, 1.093u"°"),
			Rotation(nothing, 0.31891023u"d", 0))
const deimos = Body(
			"Deimos", 1.4762u"kg" * big(10)^15, Ellipsoid(7.5u"km", 6.1u"km", 5.5u"km"), 0.068, # not clear if this is bond albedo
			Orbit(mars, 23463.2u"km", 0.00033, .93u"°"),
			Rotation(nothing, 1.263u"d", 0))

const ceres = Body(
			"Ceres", 9.3835u"kg" * big(10)^20, Sphere(469.73u"km"), 0.09, # this is geometric albedo; bond albedo not available
...
const neptune = Body(
			"Neptune", 1.02413u"kg" * big(10)^26, Spheroid(24764u"km", 24341u"km"), 0.29,
			Orbit(sol, 30.07u"AU", 0.008678, 6.43, 131.784, 276.336),
			Rotation(0.23, 0.6713u"d", 28.32)) # tilt to orbit

const pluto = Body(
			"Pluto", 1.303u"kg" * big(10)^22, Sphere(1188.3u"km"), 0.575, # average geometric; bond albedo not available
			Orbit(sol, 39.482u"AU", 0.2488, 11.88, 110.299, 113.834),
			Rotation(nothing, 6.3872304u"d", 122.53)) # tilt to orbit
const charon = Body(
			"Charon", 1.586u"kg" * big(10)^21, Sphere(606u"km"), 0.35, # average geometric; bond albedo not available
			Orbit(pluto, 19591.4u"km", 0.0002, 0.08, 223.046, nothing), # SMA to centre of Pluto, not to common barycenter
